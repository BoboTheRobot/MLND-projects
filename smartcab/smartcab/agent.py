...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        self.epsilon = math.e**-(0.01 * self.curT)
        #self.epsilon = 0.9 ** self.curT
        #self.alpha = 0.3 + (1.0 - (math.e** -(0.05 * self.curT))) * 0.6
        self.curT += 1
    ...
    def get_maxQ(self, state):
        ...
        if state in self.Q:
            maxQval = max(self.Q[state].items(), key=lambda x: x[1])
    ...
    def choose_action(self, state):
        ...
        if (self.learning == False):
            ...
            if (random.random() <= self.epsilon):
                ...
                maxQactions = self.get_maxQ(self.state)
                if (maxQactions != None):
...
def run():
    ...
    #sim = Simulator(env, update_delay=2, log_metrics=True, optimized=True, display=True)
